...
const defaultMovies = [
  { id: 1, title: "Midnight Chase", genre: "Action • Thriller", duration: "2h 18m", rating: 8.2 },
  { id: 2, title: "Galaxy Beyond", genre: "Sci-Fi • Adventure", duration: "2h 25m", rating: 7.6 },
  { id: 3, title: "The Last Melody", genre: "Drama • Music", duration: "2h 05m", rating: 8.7 },
  { id: 4, title: "Campus Days", genre: "Comedy • Romance", duration: "2h 10m", rating: 6.9 },
  { id: 5, title: "Shadow Protocol", genre: "Action • Mystery", duration: "2h 20m", rating: 7.1 },
  { id: 6, title: "Ocean Lights", genre: "Animation • Family", duration: "1h 48m", rating: 8.9 },
];
const cities = ["Bengaluru", "Chennai", "Hyderabad", "Mumbai", "Delhi NCR", "Pune", "Kolkata"];
...
let tickets = 1;
let selectedSeats = [];
let selectedCity = read("city", null) || cities[0];
let minRating = 0;
...
const ratingOf = (m) => Number(m.rating) || 0;
const stars = (m) => `⭐ ${ratingOf(m).toFixed(1)}/10`;

/* ---------- city ---------- */
function renderCity() {
  const list = [...new Set([...cities, ...Object.values(locations)])];
  if (!list.includes(selectedCity)) selectedCity = list[0];
  el("citySelect").innerHTML = list
    .map((c) => `<option ${c === selectedCity ? "selected" : ""}>📍 ${c}</option>`)
    .join("");
  el("cityNote").textContent = `Showing movies and theatres for ${selectedCity}.`;
}
function pickCity(v) {
  selectedCity = v.replace("📍 ", "").trim();
...
function pickRating(r) {
  ...
  document
    ...
    .forEach((c) => c.classList.toggle("active", Number(c.dataset.rating) === r));
  renderMovies();
}
...
function renderMovies() {
  ...
  const list = movies.filter(
    (m) =>
      ...
      ratingOf(m) >= minRating
  );
  el("movieGrid").innerHTML = list.length
    ? list
        .map(
          (m) => `<div class="movie"><div class="poster"><span class="rating">${stars(m)}</span><h3>${m.title}</h3></div>
      <div class="movie-info"><p>${m.genre} • ${m.duration}</p>
      <button onclick="selectMovie(${m.id})">Book Now</button></div></div>`
        )
        .join("")
    : `<p class="hint">No movies match your search or rating filter.</p>`;
}
...
function updateBooking() {
  ...
  el("summaryMovie").textContent = `${m.title}  ${stars(m)}`;
  el("summaryDetails").textContent =
    `📍 ${selectedCity} • ${theatre} • ${locations[theatre] || "City Centre"} • ${selectedTime} • Seats: ` +
    ...
    <div class="ticket">
      <div class="ticket-top">
        <div>
          ...
          <h3>${b.movie}</h3>
          <p>${b.theatre}</p>
          <p class="muted">📍 ${b.location}${b.city ? " (" + b.city + ")" : ""}</p>
        </div>
...
function addMovie() {
  ...
  movies.push({
    ...
    rating: Math.min(10, Math.max(0, Number(el("movieRating")?.value) || 7)),
  });
  save();
  populate();
  renderMovies();
  ...
  if (el("movieRating")) el("movieRating").value = "";
}
...
function renderAdmin() {
  el("adminMovies").innerHTML = movies
    .map(
      (m) => `<div class="admin-item"><span><b>${m.title}</b><br><small>${m.genre} • ${stars(m)}</small></span>
...
/* ---------- init ---------- */
/* backfill ratings for movies stored before ratings existed */
if (movies.some((m) => !m.rating)) {
  movies = movies.map((m) => (m.rating ? m : { ...m, rating: 7.5 }));
  save();
}
renderCity();
populate();
